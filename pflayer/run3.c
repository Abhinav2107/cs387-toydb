...
error = PF_ReadSnapshot(fd0, sfd0, 14, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
error = PF_ReadSnapshot(fd0, sfd0, 1, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
error = PF_ReadSnapshot(fd1, sfd1, 5, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
error = PF_ReadSnapshot(fd4, sfd4, 1, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
error = PF_ReadSnapshot(fd4, sfd4, 7, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
PF_UnfixPage(fd1, 4, 0);
error = PF_ReadSnapshot(fd3, sfd3, 6, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
error = PF_GetThisPage(fd4, 14, &ptr, &pagebuf);
PF_UnfixPage(fd4, 14, 0);
error = PF_ReadSnapshot(fd4, sfd4, 8, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
error = PF_ReadSnapshot(fd4, sfd4, 4, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
PF_UnfixPage(fd3, 6, 1);
error = PF_ReadSnapshot(fd3, sfd3, 12, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
error = PF_ReadSnapshot(fd4, sfd4, 0, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
PF_UnfixPage(fd4, 8, 0);
error = PF_ReadSnapshot(fd0, sfd0, 12, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
}
...
PF_UnfixPage(fd1, 0, 0);
error = PF_ReadSnapshot(fd2, sfd2, 1, &ptr, &pagebuf);
error = PF_GetThisPage(fd4, 13, &ptr, &pagebuf);
if(error == PFE_DISK) {
    while(fetchPFRAID_buf(ptr) != PFE_FETCHED)
        stepTime();
